...
setup_kernelsu() {
    ...
    test -d "$GKI_ROOT/KernelSU" || git clone https://github.com/xvergilantte/KernelSU && echo "[+] Repository cloned."
    cd "$GKI_ROOT/KernelSU"
    git stash && echo "[-] Stashed current changes."
    if [ "$(git status | grep -Po 'v\d+(\.\d+)*' | head -n1)" ]; then
        git checkout legacy-susfs && echo "[-] Switched to legacy-susfs branch."
    fi

...
static void ksu_hack_lsm_slot(struct hlist_head *hook_head, uintptr_t *old_ptr, uintptr_t new_ptr)
{
	...
	uintptr_t addr = (uintptr_t)&pos->hook;
	...
	WRITE_ONCE(*target_slot, (void *)new_ptr);
...
do {														\
	pr_info("LSM: Initializing hack for hook '%s'\n", #hook_name);						\
...
static __init void ksu_lsm_hook_init(void)
{
	...
	LSM_HACK_INIT(setprocattr, ksu_setprocattr_wrapper);
